...
const pages = ['counting machine', 'calculator', 'BMI', 'board'];
const preSettings = ['sign up', 'sign in'];
...
export function Nav(){
  ...
  useEffect(() => {
    ...
    if (loginUser === null) {
      ...
      setUserSubTitle(["sign up","sign in"])
    } else {
  ...
  const handleClick = (value) => { 
    switch(value) {
      case 'counting machine' : window.location.href='/basic/counter'
                      break;
      case 'calculator' : window.location.href='/basic/calc'
                      break;
  ...
  const handleAuth = (value) => {
    switch(value) {
      case 'sign up' : window.location.href='/user/join'
                      break;
      case 'sign in' : window.location.href='/user/login'
                      break;
      default: window.location.href='/'
                      break;
    }
          ...
          <Box sx={{ flexGrow: 0 }}>
            <Tooltip title="Open settings">
              <IconButton onClick={handleOpenUserMenu} sx={{ p: 0 }}>
                <Avatar alt="Remy Sharp" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRO9vpQjCU7BAbg2kgPZC_WCeHfq5NqdmpMzg&usqp=CAU" />
              </IconButton>
            </Tooltip>
            <Tooltip title="Open settings">
              <IconButton onClick={handleOpenUserMenu} sx={{ p: 0 }}>
                <Avatar alt="Remy Sharp" src="http://www.coinreaders.com/imgdata/coinreaders_com/202201/202201073433499.jpeg" />
              </IconButton>
            </Tooltip>
            <Menu
              sx={{ mt: '45px' }}
              ...
              onClose={handleAuth}
            >
              {preSettings.map((setting) => ( // sign up, sing in
